...
public class NhanVienDao {
    ...
    public List<NhanVien> listAll() {
        ...
        try (Cursor c = db.rawQuery(
                "SELECT nv.nv_id, nv.ten, nv.sdt, nv.email, nv.diachi, nv.chucvu, " +
                        "nv.ngay_vao_lam, nv.trangthai, nv.user_id, u.username " +
                        "FROM NhanVien nv LEFT JOIN Users u ON u.user_id = nv.user_id " +
                        "ORDER BY nv.nv_id DESC", null)) {
    ...
    public int updateWithUser(NhanVien nv, String username, String password) {
        SQLiteDatabase db = helper.getWritableDatabase();
        db.beginTransaction();
        try {
            int userId = nv.userId;
            if (userId > 0) {
                ContentValues u = new ContentValues();
                u.put("username", username);
                u.put("role", inferRole(nv.chucvu));
                u.put("status", 1);
                if (com.example.quanlythuvien.util.StaffAccountValidator.shouldUpdatePassword(password)) {
                    u.put("password", password);
                }
                int userRows = db.update("Users", u, "user_id = ?",
                        new String[]{String.valueOf(userId)});
                if (userRows <= 0) return -1;
            } else {
                ContentValues u = new ContentValues();
                u.put("username", username);
                u.put("password", password);
            ...
            int rows = db.update("NhanVien", cv, "nv_id = ?",
                    new String[]{String.valueOf(nv.nvId)});
            if (rows <= 0) return -1;
            db.setTransactionSuccessful();
            return rows;
